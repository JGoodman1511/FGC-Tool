# Full source for FGC Kivy country database application (Optimized: Parallel API/downloads, batched UI, debounced selects; Fixed ComboBox open; Updated dashboard formatting; Scaled reference image with no top padding; Flashcard text styling matches dashboard with white text and constant flag size; Dashboard panels with dynamic text box height based on text wrapping in country name and pronunciation, with row-by-row synchronized heights for symmetric columns and dynamic scaling on window resize; Dynamic font size reduction for country name (36sp/28sp) and pronunciation (20sp/16sp) when text wraps, with larger country name and much smaller pronunciation; Prioritize font shrinking over wrapping for non-space-delimited text; Fixed database build to fetch API data for all non-hardcoded countries, handle duplicates, and prevent application exit)
...
class MainPanel(BoxLayout):
    ...
    def _add_country_panel(self, container, country, code, continent, pron, flag_path, bg_color, is_right_column=False):
        ...
        name_label = make_dynamic_label(country, '36sp', (0, 0, 0, 1), bold=True, min_height=dp(40))
